 ...
 */
#define LVGL_PORT_TICK_PERIOD_MS (5) // Slightly lower LVGL tick pressure for better overall system stability
...
#define LVGL_PORT_TASK_MIN_DELAY_MS (4)        // Keep LVGL responsive while yielding CPU to Wi-Fi/voice tasks
#define LVGL_PORT_TASK_STACK_SIZE   (6 * 1024) // The stack size of the LVGL timer task, in bytes
#define LVGL_PORT_TASK_PRIORITY     (configMAX_PRIORITIES - 2) // Leave headroom for critical driver/system tasks
#ifdef ARDUINO_RUNNING_CORE
#define LVGL_PORT_TASK_CORE (ARDUINO_RUNNING_CORE) // Valid if using Arduino
#elif defined(CONFIG_FREERTOS_UNICORE)
#define LVGL_PORT_TASK_CORE (0)
#else
#define LVGL_PORT_TASK_CORE (0) // Keep LVGL on the same core as board->begin() to reduce screen drift
#endif
...
#else
// Keep default conservative to preserve PSRAM headroom for voice pipeline models.
#define LVGL_PORT_AVOID_TEARING_MODE (0) // Valid if using Arduino / fallback
#endif
