...
static void uart_json_task(void* arg)
{
    ...
    while (1) {
        uart_event_t evt;
        if (xQueueReceive(uart_json_queue, &evt, portMAX_DELAY) != pdTRUE)
            continue;
...
int uart_json_init(uart_json_on_text_cb_t on_text)
{
    ...
    esp_err_t r =
        uart_manager_install(UART_JSON_NUM, UART_JSON_RX_PIN, UART_JSON_TX_PIN, UART_JSON_BAUD, &uart_json_queue);
    if (r != ESP_OK) {
