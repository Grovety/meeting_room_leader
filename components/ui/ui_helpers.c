...
// LVGL version: 8.3.11
// Project name: clock5.1

#include "ui_helpers.h"

static void _ui_destroy_screen_by_pointer(lv_obj_t *screen)
{
    if (!screen) {
        return;
    }

    if (screen == ui_Screen8) {
...
void _ui_screen_change(lv_obj_t ** target, lv_scr_load_anim_t fademode, int spd, int delay, void (*target_init)(void))
{
    ...
    lv_obj_t *scratch = NULL;
    if (current != NULL && current != *target) {
        scratch = lv_obj_create(NULL);
        lv_obj_clear_flag(scratch, LV_OBJ_FLAG_SCROLLABLE);
        lv_obj_set_style_bg_color(scratch, lv_color_hex(0x000000), LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_bg_opa(scratch, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_scr_load_anim(scratch, LV_SCR_LOAD_ANIM_NONE, 0, 0, false);
        _ui_destroy_screen_by_pointer(current);
    }
